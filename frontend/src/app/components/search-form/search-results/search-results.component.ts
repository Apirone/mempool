...
export class SearchResultsComponent implements OnChanges {
  ...
  ngOnChanges() {
    this.activeIdx = 0;
    if (this.results) {
      // ...this.results.pools, if you need
      this.resultsFlattened = [...(this.results.hashQuickMatch ? [this.results.searchText] : []), ...this.results.addresses, ...this.results.nodes, ...this.results.channels, ...this.results.otherNetworks];
